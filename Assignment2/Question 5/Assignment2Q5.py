...
train_image = imageio.imread(sys.path[0]+"/img1.jpg") 
query_image = imageio.imread(sys.path[0]+"/img2.jpg") 

gtrain_image = cv2.cvtColor(train_image, cv2.COLOR_RGB2GRAY)
gquery_image = cv2.cvtColor(query_image, cv2.COLOR_RGB2GRAY)

kps_train, features_train = KeyPointsFeatures(gtrain_image) 
kps_query, features_query = KeyPointsFeatures(gquery_image) 

matches = MatchKeyPoints(features_train, features_query, 0.75)
temp_img = cv2.drawMatches(train_image, kps_train, query_image, kps_query, np.random.choice(matches,100), None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
...
result = cv2.warpPerspective(train_image, homography, (width, height))
